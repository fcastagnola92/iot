...
''' 
Filtra los datos por fecha si se proporciona, extrae las mediciones y sus valores, y devuelve los resultados como una respuesta JSON.
'''
def get_list_measurement_by_date(request, **kwargs):
    data_result = []
    selectedData = None
    dateParam = kwargs.get("date", None)

    if dateParam != None:
        selectedData = Data.objects.filter(base_time__gte=dateParam).all()
    else:
        selectedData = None
